...
import { saveFeedback, getHistory, getSubjectStats } from './services/storageService';
...
const DRAFT_KEY = "isgi_feedback_draft";
    ...
    <div className="fixed inset-0 z-[200] bg-slate-950 flex flex-col">
      <div className="p-6 flex items-center justify-between border-b border-slate-900 bg-slate-950/80 backdrop-blur-md">
        <h3 className="font-black text-xs uppercase tracking-[0.3em] text-white flex items-center gap-3">
          <Camera className="w-4 h-4 text-indigo-400" /> Scanner Module GI
        </h3>
...
const App: React.FC = () => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ subject: '', q1: null, q2: null, q3: null, q4: null, q5: null, q6_jobs: null, q7_rooms: null, q8_resources: null, q9_transport: null, q10_laptop: null, comments: '' });
      setStep('thanks');
    } catch { 
      setStep('thanks'); 
    }
  ...
  const totalSteps = GI_SUBJECTS.length + 1; // 10 subjects + 1 Env Audit
  const currentProgressCount = completedSubjects.length + (envAuditDone ? 1 : 0);
  const globalProgression = Math.round((currentProgressCount / totalSteps) * 100);
  ...
  return (
    <div className="min-h-screen industrial-pattern text-slate-100 pb-20 selection:bg-indigo-500/30">
      ...
      <div className={`fixed inset-y-0 left-0 z-[150] bg-slate-950 border-r border-slate-900 transform transition-all duration-500 shadow-2xl flex flex-col w-80 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 flex items-center justify-between border-b border-slate-900">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-black text-[10px] uppercase tracking-[0.25em] text-white">Console Admin</h3>
          </div>
        ...
        <div className="flex-1 p-6 flex flex-col justify-center items-center text-center space-y-6">
          ...
          <p className="text-[10px] font-black uppercase text-slate-600 tracking-widest">Accès réservé aux auditeurs qualité</p>
        </div>
      </div>
      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
        {step === 'welcome' && (
          <div className="py-20 text-center space-y-12 animate-in fade-in duration-700">
            ...
            <div className="space-y-4">
              ...
              <p className="text-slate-500 font-bold uppercase tracking-[0.3em] text-[10px]">Maintenance • Qualité • Performance</p>
            </div>
            <button onClick={() => setStep('hub')} className="w-full py-10 bg-indigo-600 hover:bg-indigo-500 rounded-[40px] font-black uppercase text-white shadow-2xl border-b-8 border-indigo-800 transition-all active:translate-y-1 hover:scale-[1.02]">Démarrer l'Audit de Session</button>
          </div>
        )}
          ...
          <div className="space-y-10 animate-in slide-in-from-bottom-6 duration-500">
              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <div className="bg-indigo-500/20 p-2 rounded-xl border border-indigo-500/20 shadow-lg">
                      <Flame className="w-5 h-5 text-indigo-400" />
                    </div>
                    <p className="text-[10px] font-black uppercase tracking-[0.4em] text-indigo-400">Progression Globale ISGI</p>
                  </div>
                  <h2 className="text-4xl md:text-5xl font-black text-white italic tracking-tighter uppercase leading-none">
                    {currentProgressCount} <span className="text-slate-600">/ {totalSteps}</span>
                  </h2>
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                    Étapes de l'audit complétées
                  </p>
                </div>
                ...
                <div className="flex-1 max-w-sm w-full space-y-4">
                  <div className="flex justify-between items-end px-1">
                    ...
                    <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest">OBJECTIF 100%</span>
                  </div>
                  <div className="h-6 bg-slate-950 rounded-full border-2 border-slate-800 p-1 relative shadow-inner overflow-hidden">
                    <div 
                      className="h-full bg-gradient-to-r from-indigo-600 to-indigo-400 rounded-full transition-all duration-1000 shadow-[0_0_20px_rgba(99,102,241,0.4)] relative" 
                  ...
                  {globalProgression === 100 ? (
                    <div className="flex items-center gap-2 text-emerald-400 animate-in zoom-in">
                      <AwardIcon className="w-4 h-4" />
                      <span className="text-[9px] font-black uppercase tracking-widest">Audit Qualité Terminé - Merci</span>
                    </div>
                  ) : (
                    <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest">
                      Continuez pour finaliser le diagnostic de l'institut
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-3xl font-black uppercase italic leading-none">Modules</h2>
              </div>
              <button onClick={() => setStep('scanner')} className="p-4 bg-indigo-500/10 border border-indigo-500/30 rounded-2xl text-indigo-400 flex items-center gap-3 font-black text-[10px] uppercase hover:bg-indigo-500 hover:text-white transition-all shadow-lg shadow-indigo-500/10"><QrCode className="w-5 h-5" /> Scanner QR</button>
            </div>
            ...
            <div className="bg-slate-900/20 border border-slate-800/60 rounded-[40px] p-8 space-y-6 backdrop-blur-md">
              <div className="flex items-center justify-between px-2">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Statut des Diagnostics</p>
                </div>
              </div>
            ...
            {firstUncompleted && (
              <button onClick={() => startPedagogy(firstUncompleted)} className="w-full p-8 rounded-[40px] bg-indigo-600 border-2 border-indigo-500 hover:bg-indigo-500 transition-all flex items-center justify-between shadow-2xl group active:translate-y-1">
                <div className="flex items-center gap-5 text-left">
                   ...
                   <div>
                     <h3 className="text-xl font-black text-white uppercase leading-none">Diagnostic Suivant</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pb-12">
              <button onClick={() => setStep('modules')} className="p-10 rounded-[40px] bg-slate-900/40 border-2 border-slate-800 hover:border-indigo-500/50 transition-all text-left group">
                ...
                <h3 className="text-2xl font-black uppercase text-white">Modules GI</h3>
                <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mt-2">Inventaire des Cours</p>
              </button>
              <button onClick={startEnvAudit} className={`p-10 rounded-[40px] border-2 transition-all text-left group ${envAuditDone ? 'bg-emerald-500/5 border-emerald-500/30' : 'bg-slate-900/40 border-slate-800 hover:border-emerald-500/50'}`}>
                ...
                <p className={`text-[10px] font-black uppercase tracking-[0.3em] mt-2 ${envAuditDone ? 'text-emerald-400' : 'text-slate-500'}`}>{envAuditDone ? 'Audit Clôturé' : 'En attente'}</p>
              </button>
            </div>
          </div>
        )}
        ...
        {step === 'modules' && (
          <div className="space-y-6 animate-in fade-in duration-500 pb-20">
            <div className="flex items-center justify-between mb-8"><h2 className="text-3xl font-black uppercase italic">Catalogue Modules</h2><button onClick={() => setStep('hub')} className="p-4 bg-slate-900 rounded-2xl text-slate-500 hover:text-white transition-colors"><ArrowLeft className="w-5 h-5" /></button></div>
        ...
        {(step === 'form_pedagogy' || step === 'form_env') && (
          <form onSubmit={handleFormSubmit} className="space-y-10 pb-40 animate-in slide-in-from-bottom-10 duration-500">
            <div className={`p-10 rounded-[48px] border-2 relative overflow-hidden shadow-2xl ${step === 'form_env' ? 'bg-emerald-600/10 border-emerald-500/30' : 'bg-indigo-600/10 border-indigo-500/30'}`}>
              ...
              <p className={`text-[10px] font-black uppercase tracking-[0.4em] mb-4 ${step === 'form_env' ? 'text-emerald-400' : 'text-indigo-400'}`}>{formData.subject === 'ENVIRONNEMENT_GLOBAL' ? 'Audit Infrastructure' : 'Audit Pédagogique'}</p>
            ...
            <div className="p-10 bg-slate-900/40 rounded-[48px] border-2 border-slate-800 shadow-xl">
               ...
               <textarea value={formData.comments} onChange={e => setFormData({...formData, comments: e.target.value})} placeholder="Détaillez vos remarques ou suggestions d'amélioration..." className="w-full h-40 bg-slate-950 border-2 border-slate-800 rounded-3xl p-8 text-sm focus:border-indigo-500 outline-none transition-all placeholder:text-slate-800" />
            </div>
        ...
        {step === 'submitting' && (
          <div className="fixed inset-0 z-[200] bg-slate-950/95 flex flex-col items-center justify-center backdrop-blur-2xl">
             ...
             <div className="text-center space-y-4">
                <p className="text-3xl font-black text-white uppercase italic tracking-tighter">Transmission Sécurisée</p>
                <div className="flex flex-col items-center gap-1">
                  <p className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-500 animate-pulse">Analyse Diagnostique IA en cours</p>
                  <p className="text-[8px] font-mono text-slate-700">MODÈLE: GEMINI-3-FLASH-PREVIEW</p>
                </div>
             </div>
          </div>
        )}
        ...
        {step === 'thanks' && (
          <div className="py-20 text-center space-y-16 animate-in zoom-in duration-700">
            ...
            <div className="max-w-md mx-auto space-y-4">
              <button onClick={() => setStep('hub')} className="w-full py-10 bg-indigo-600 hover:bg-indigo-500 rounded-[40px] font-black uppercase tracking-widest text-white shadow-xl hover:scale-105 active:scale-95 transition-all">Retour au Hub Opérationnel</button>
              <p className="text-slate-600 font-bold uppercase text-[9px] tracking-widest italic">Votre feedback contribue à l'amélioration continue de l'institut.</p>
            </div>
          </div>
        )}
      </main>
      ...
      <footer className="py-12 text-center opacity-30 border-t border-slate-900/50 mt-20">
        <p className="text-[10px] font-black text-slate-700 uppercase tracking-[0.6em] mb-2">Institut de Génie Industriel • ISGI GIM</p>
        <p className="text-[8px] font-mono text-slate-800 uppercase tracking-widest">© 2024 • Système d'Évaluation de la Qualité Académique</p>
      </footer>
    </div>
  );
};
